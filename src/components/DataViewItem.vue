<template>
  <GenericItem :name="name" :data="data" :readonly="readonly">
    <template v-slot:buttons>
      <VButton
        class="edit-button button-outline-secondary button-sm"
        @click="onEdit"
      ...
      <VButton
        class="remove-button button-outline-danger button-sm"
        @click="onRemove"
      >
        <svg
          class="w-6 h-6 text-gray-800 dark:text-white"
...
const structure = computed(
  () => new MatrixStructure(name.value, data.value, readonly.value)
);

function onEdit() {
  console.info("DataViewItem.onEdit");
  emit("edit", structure.value);
}

function onRemove() {
  console.info("DataViewItem.onRemove");
